...
// La exportación de JSON ahora debe recibir los datos
// import { exportarAuditoriaJSON } from '../servicios/api'; 
import { ACCIONES_AUDITORIA } from '../utilidades/constants';

// Función helper movida aquí o importada
...
// El componente ahora recibe los eventos como prop
const TablaAuditoria = ({ eventos = [] }) => {
  ...
  const estadisticas = useMemo(() => {
    if (!eventos || eventos.length === 0) {
      return { totalEventos: 0, eventosPorAccion: {}, eventosPorUsuario: {}, primerEvento: null, ultimoEvento: null };
    }
    ...
    return {
      ...
      eventosPorAccion,
      eventosPorUsuario,
      primerEvento: eventos[eventos.length - 1]?.timestamp, // Invertido
      ultimoEvento: eventos[0]?.timestamp // Invertido
    };
  }, [eventos]);
  ...
  useEffect(() => {
    ...
    if (busqueda) {
      resultado = resultado.filter(evento =>
        (evento.detalle || '').toLowerCase().includes(busqueda.toLowerCase()) ||
        (evento.usuario?.username || '').toLowerCase().includes(busqueda.toLowerCase()) || // Asumiendo que usuario es un objeto o string
    ...
    if (filtroUsuario !== 'todos') {
      resultado = resultado.filter(evento => evento.usuario?.username === filtroUsuario); // Ajustado para objeto de usuario
    }
  ...
  const obtenerUsuariosUnicos = () => {
    // Asumiendo que `evento.usuario` es un objeto { id, username }
  ...
  // (Las funciones obtenerColorAccion y obtenerIconoAccion permanecen igual que en tu archivo)
    ...
    <div className="animacion-entrada">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          ...
          <div className="tarjeta">
           ...
           <div className="tarjeta">
            ...
            <p className="texto-sm font-bold" style={{ color: '#f59e0b', marginTop: '0.5rem' }}>
              {estadisticas.ultimoEvento ? 
                new Date(estadisticas.ultimoEvento).toLocaleString('es-CL') : 'N/A'}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="tarjeta mb-6">
        <div className="grid grid-cols-4 gap-4">
          {/* ... (input de búsqueda, igual que en tu archivo) ... */}
      ...
      {/* Tabla de eventos */}
      <div className="tarjeta">
        <h3 className="texto-xl font-bold">Registro de Auditoría ({eventosFiltrados.length})</h3>
      ...
      {/* Modal de detalle (igual que en tu archivo) */}
      {eventoSeleccionado && (
         <div style={{
          ...
          <div className="tarjeta" style={{ 
            ...
            <div className="grid gap-4">
              ...
              <div>
                ...
                <span style={{
                  ...
                  fontWeight: 'bold', backgroundColor: obtenerColorAccion(eventoSeleccionado.accion) + '20',
            ...
            <button
              ...
              className="boton boton-primario mt-6" style={{ width: '100%' }}
            >
              Cerrar
            </button>
          </div>
